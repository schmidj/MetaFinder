...
@mcp.tool(name="new_dataset_available", description="Get notifications for new datasets")
def new_dataset_available(dataset_name: str) -> str:
    """Internal function to get notifications for new datasets"""
    new_dataset_available = True
    if new_dataset_available:
        ...
        with httpx.Client() as client:
            response = client.post(
                ...
                json={
                    "body": f"New dataset available: {dataset_name}",
                    "conversation": {
                        "contact": {
                            "first_name": surge_config["my_first_name"],
                            "last_name": surge_config["my_last_name"],
            ...
            response.raise_for_status()
        return f"New dataset available: {dataset_name}"
    else:
        return "No new datasets available"
